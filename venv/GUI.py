...
def liveFeed():
    ...
    def detected(response, img):
        window.destroy()
        list = response.json()
        name = list[0] + " " + list[1] + " " + list[2]
        record = list[3]
        alert(name, record, img)
...
def alert(name, record, img):
    ...
    frame.grid(row=4, column=0, pady=5)
    btnOk = Button(master=frame, text='Ok', font=('roboto', 25), width=30, bd=5, bg='#81D6E1', command=None)
